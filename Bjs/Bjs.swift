...
public class Bjs {
    ...
    public func putWrapped(_ native: Any?, _ nativeWrapperClass: BjsNativeWrapper.Type) -> JSValue {
        ...
        if let jsObj = context.createJsObject(native) {
            ...
            if jsWrapperObj == nil || jsWrapperObj!.isUndefined {
                jsObj.setObject(Bjs.bjsWrapperObjFieldUnboundValue,
                                forKeyedSubscript: Bjs.bjsWrapperObjFieldName as NSString)
                let newJsWrapperObj = self.loadModule(nativeWrapperClass.bjsLocator.moduleName)
                    .construct(withArguments: [jsObj])!
    ...
    public func getArray<T>(_ jsArrayObject: JSValue, _ elementConverter: (_ jsElement: JSValue) -> T) -> [T]? {
        ...
        if jsArrayObject.isArray && lengthJs.isNumber {
            ...
            if arrayLength > 0 {
                for itemIndex in 0...arrayLength - 1 {
                    bjsArray.append(elementConverter(jsArrayObject.atIndex(itemIndex)!))
                }
            }
        } else {
    ...
    public func bindNative(_ native: Any!, _ wrapper: JSValue) {
    ...
    func loadModule(_ moduleName: String) -> JSValue {
        ...
        if module.isUndefined {
            fatalError("JS module \"\(moduleName)\" must be exported " +
                       "with the notation \"module.exports = {\(moduleName)}\"")
        }
    ...
    func createNativeObj(_ jsObj: JSValue, _ bjsObj: BjsObject) {
        ...
        context.logError("bjs object \"\(String(describing: bjsObj.self))\" was initialized " +
                         "with a js object already bound with another bjs object")
    }
}
